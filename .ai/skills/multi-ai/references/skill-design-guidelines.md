...
### Anti-Pattern: Using @content.md directive instead of inline content
**Novice**: "I'll write `@content.md` in SKILL.md so it stays small and the harness resolves the reference at load time."
**Expert**: The `@content.md` directive was the original approach but is no longer used. The build script now embeds the content of `content.md` directly into each `SKILL.md` at build time. Writing `@content.md` manually would leave harnesses with an unresolved directive instead of actual skill instructions.
**Timeline**: Pre-2026 (`@content.md` directive): SKILL.md contained only frontmatter + `@content.md` reference → 2026+: build script embeds content inline; `@content.md` directive is never written.
**LLM mistake**: Models trained on the old SKILL.md format may reproduce `@content.md` when asked to generate or repair a SKILL.md file.
**Detection**: Any SKILL.md whose body is exactly `@content.md` (after the closing `---`) was generated by the old build system — re-run `build-skills.py` to regenerate with inline content.
...
**Detection**: `find .claude/skills -type l -name content.md` returns any results — symlinks at that path indicate the old build approach was used and `build-skills.py` has not been re-run to clean them up.
