...
For each subdirectory <name> in .ai/skills/:
  ...
  2. For each harness in [claude, codex, cursor, copilot]:
     ...
     c. Remove <target>/content.md symlink if it exists
        (cleanup of the former @content.md approach)
        If a non-symlink file exists at that path: print warning, leave it alone

     d. For every file or subfolder inside .ai/skills/<name>/
     ...
     f. Build SKILL.md content:
          ---\n<contents of <harness>.yaml>---\n\n<contents of content.md>

     g. Write <target>/SKILL.md only if content differs from existing file

  3. Print summary: skills processed, SKILL.md files written, symlinks created/verified/removed, errors
```
...
- Harness `SKILL.md` contains frontmatter block followed by the full content body (no `@content.md` directive)
- No `content.md` files exist in harness skill folders — they are no longer created by the build script
- SKILL.md content matches: frontmatter from `<harness>.yaml` + body from `.ai/skills/<name>/content.md`
