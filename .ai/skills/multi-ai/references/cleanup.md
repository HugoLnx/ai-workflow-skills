...
After disabling one or more harnesses in `.ai/config.yml`, run cleanup to remove their stale output. The script also detects legacy folders (`.cursor/rules/`, `.github/integrations/`) that should not exist in a multi-ai project.
...
| `.github/integrations/` | Legacy Copilot integrations directory — not used by multi-ai |
