...
```
lnx-grill-me         | claude !codex cursor !copilot
...
**Novice**: "I'll install `.claude/skills/lnx-grill-me/` globally — that's the skill folder."
...
**Detection**: More than one `lnx-ai-global-skills.py install` call for the same skill in one session.
