...
The canonical cross-OS implementation is at `.ai/skills/cross-skills/scripts/build-skills.py`.
Run it with `python .ai/skills/cross-skills/scripts/build-skills.py`.

Key implementation notes:
- Resolve `REPO_ROOT` via `Path(__file__).resolve().parents[4]` (script sits 4 dirs deep)
...
```gitignore
# Generated by build-skills script — rebuild with: python .ai/skills/cross-skills/scripts/build-skills.py
