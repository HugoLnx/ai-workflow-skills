...
Rules follow the same pattern: source in `.ai/rules/`, distributed to `CLAUDE.md`, `AGENTS.md`, `.cursor/rules/`, and `.github/instructions/` via the rules build script.

## Project Structure

- `.ai/skills/` — skill source files (single source of truth)
- `.ai/rules/` — rule source files (single source of truth)
...
- Edit rules in `.ai/rules/`, then rebuild: `python .ai/skills/multi-ai/scripts/build-rules.py`
...
- `@multi-ai` — manage skills and rules across all harnesses; edit `.ai/`, design, build, migrate, validate
